...
- Можно ли переводить PR в Ready for review: нет. GitHub уже закрыл и слил PR #2 автоматически после попытки создать обычный Open PR.
...
- PR #1 не удалось переоткрыть после force-push.
- PR #2 был создан как обычный Open PR, но GitHub затем закрыл и слил его автоматически.
...
Нет. Текущий PR #2 уже закрыт и merged, поэтому нужен новый branch/PR, если требуется открытый reviewable PR.
