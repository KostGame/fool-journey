...
## SU-017 result feedback smoke-test

- открыть Pages и пройти несколько сцен;
- выбрать действие и убедиться, что result screen показывает выбранное действие, outcome tone и краткое изменение сцены;
- проверить, что блоки `Получено в Карты Шута`, `Применено из Карт Шута` и `Помощник пути` читаются явно;
- проверить, что пояснение рядом с картой говорит, зачем она нужна дальше;
- убедиться, что inventory panel на сцене стал понятнее и не перегружает экран;
- проверить, что journal, thumbnails и основной flow не сломались;
- снова убедиться в отсутствии горизонтальной прокрутки на 360-430 px.
